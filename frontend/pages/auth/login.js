      ...
      .then((response) => {
        ...
        if (response?.result_code === 0) {
          ...
          notificationWithIcon(
            'error',
            'ERROR',
            response?.result?.message || 'Sorry! Something went wrong. App server error'
          );
        }
      })
      .catch((err) => {
        ...
        let message = 'Sorry! Something went wrong. App server error';

        if (err?.response?.data?.result?.error) {
          const errorObj = err.response.data.result.error;
          message =
